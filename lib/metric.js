 ...
 */
 ...
 Metric.prototype._batchBufferGet = function () {
   return this._batchTimeSeries || []
 }
 Metric.prototype._batchBufferPush = function (timeSeriesItem) {
   if (this._batchTimeSeries) {
    this._batchBufferUpdate(timeSeriesItem)
   } else {
     this._batchTimeSeries = [timeSeriesItem]
   }
 }
...
Metric.prototype._queueTimeSeriesItem = function (timeSeriesItem) {
  ...
  if (this._batchPromise) {
    ...
    this._batchPromise = this._batchTimeout.then(function () {
      const timeSeries = self._batchBufferGet()
      // reset batch state
